...

{
	...
	\begin{algorithm}
		...
		\For{\(r = 0,\dots,n-1\)}{
			...
			\(\xL \leftarrow L_{\frac{r(r+1)}2+r}\)\tcp*[f]{
				\(=L_{\frac{(r+2)(r+1)}2 -1}\) i.e. on the diagonal}\;
			\(\xc = \xL^{-1} \xc\)\;
...

{
	...
	\begin{algorithm}
		...
		\(L_{\frac{n(n+1)}2 + n} \leftarrow \chol(\xS)\)\tcp*[f]{\(=L_{\frac{(n+2)(n+1)}2-1}\) i.e. on the diagonal}\;
		\KwRet{\(\xZ\)}
	\end{algorithm}
}
